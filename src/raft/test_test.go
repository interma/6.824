...
func XTestFigure8_me(t *testing.T) {
	servers := 5
	cfg := make_config(t, servers, false)
	defer cfg.cleanup()

	fmt.Printf("Test: Figure 8 ME ...\n")

	time.Sleep(time.Duration(2000) * time.Millisecond)
	leader1 := -1
	for i := 0; i < servers; i++ {
		if cfg.rafts[i] != nil {
			_, _, ok := cfg.rafts[i].Start(100)
			if ok {
				leader1 = i
				break
			}
